...
if 'SUMO_HOME' in os.environ:
    tools = os.path.join(os.environ['SUMO_HOME'], 'lib/python2.7/site-packages')
...
import traci
# phase codes based on ped_environment.net.xml
...
class Simulation:
    ...
    def _collect_waiting_times(self):
        ...
        final_list = np.concatenate((car_list, ped_list),axis=0)
        for v_id in final_list:
            if 'person' in v_id:
                ...
                road_id = traci.vehicle.getRoadID(v_id)  # get the road id where the car is located
                if road_id in incoming_roads:  # consider only the waiting times of cars in incoming roads
                    self._waiting_times[v_id] = wait_time
                else:
    ...
    def _get_state(self):
        ...
        for ped_id in ped_list:
            ...
            if ped_lane_pos < 9:
                ...
                ped_lane_cell = 1
            elif ped_lane_pos < 23:
                ped_lane_cell = 2
            elif ped_lane_pos < 30:
                ped_lane_cell = 3
            elif ped_lane_pos < 42:
                ped_lane_cell = 4
            elif ped_lane_pos < 62:
                ped_lane_cell = 5
            elif ped_lane_pos < 102:
                ped_lane_cell = 6
            elif ped_lane_pos < 162:
                ped_lane_cell = 7
            elif ped_lane_pos < 402:
                ped_lane_cell = 8
            elif ped_lane_pos <= 752:
                ped_lane_cell = 9

            if ped_lane_id == "W2TL_0":
                ped_lane_group = 0
            elif ped_lane_id == "N2TL_0":
                ped_lane_group = 2
            elif ped_lane_id == "E2TL_0":
                ped_lane_group = 4
            elif ped_lane_id == "S2TL_0":
                ped_lane_group = 6
            else:
                ped_lane_group = -1

            if ped_lane_group >= 1 and ped_lane_group <= 6:
                ped_position = int(str(ped_lane_group) + str(ped_lane_cell))  # composition of the two postion ID to create a number in interval 0-87
                valid_ped = True
            elif ped_lane_group == 0:
        ...
        for car_id in car_list:
            ...
            if lane_id == "W2TL_1" or lane_id == "W2TL_2" or lane_id == "W2TL_3":
                ...
                lane_group = 1
            elif lane_id == "N2TL_1" or lane_id == "N2TL_2" or lane_id == "N2TL_3":
                lane_group = 2
            elif lane_id == "N2TL_4":
                lane_group = 3
            elif lane_id == "E2TL_1" or lane_id == "E2TL_2" or lane_id == "E2TL_3":
                lane_group = 4
            elif lane_id == "E2TL_4":
                lane_group = 5
