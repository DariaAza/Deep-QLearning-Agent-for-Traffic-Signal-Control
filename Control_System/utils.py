...
def import_train_configuration(config_file):
    ...
    # TODO: check if gui_episodes exists otherwise will be error
    config['gui_episodes'] = json.loads(content['simulation']['gui_episodes'])
    config['total_episodes'] = content['simulation'].getint('total_episodes')
    config['max_steps'] = content['simulation'].getint('max_steps')
    config['n_v_generated'] = content['simulation'].getint('n_v_generated')
